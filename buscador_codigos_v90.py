...
"""
Buscador + Convertidor con gestión de Proveedores integrada (v90)
- v90: auditoría completa post-v89 — 3 mejoras.
    * on_emp_search (SearchView) y _on_empresa_search_p1 (TivendoIngreso):
      lógica de filtrado de combobox idéntica en ambas clases → extraída al
      helper de módulo _filter_combobox_choices(term, choices, combo, var).
    * _ir_a_paso1: método wrapper trivial (solo llamaba _mostrar_paso(1))
      → inlineado en su único llamador (botón ← Anterior del Paso 2).
    * _apply_filter (TivendoWindow): recalculaba .map(_normalize_text) sobre
      3 columnas en cada keystroke del filtro rápido → columnas normalizadas
      pre-computadas al construir df_preview en _build_preview_from_paste,
      almacenadas en _df_preview_norm para reutilización O(1) en el filtro.
    * Títulos de ventana actualizados a v90.
"""
...
def _build_search_mask(df, q_raw: str, q_ws: str, q_norm: str, exact: bool, by_barras: bool):
    """Construye la máscara booleana de búsqueda para un único término.

    Args:
        df       : DataFrame de la base (con columnas _codigo_ws, _codigo_lc, etc.)
        q_raw    : término tal como lo escribió el usuario
        q_ws     : término con espacios colapsados (sin cambiar case)
        q_norm   : término normalizado (lower + espacios colapsados)
        exact    : True → coincidencia exacta de código; False → contains
        by_barras: True → también busca en barcode_interno / barcode_externo
    """
    q_ws_lc = q_ws.lower()
    q_name  = _norm_lc(q_ws)
    if exact:
        mask_tok = df["_codigo_tokens_norm"].apply(
            lambda toks: any(tok == q_norm for tok in toks)
        )
        mask = (df["_codigo_ws"].str.lower() == q_ws_lc) | mask_tok
        if by_barras:
            mask |= (df["barcode_interno"].str.strip() == q_ws) | \
                    (df["barcode_externo"].str.strip() == q_ws)
    else:
        token_contains = (len(q_norm) >= 3) and df["_codigo_tokens_norm"].apply(
            lambda toks: any(q_norm in tok for tok in toks)
        )
        mask = (
            df["_codigo_lc"].str.contains(re.escape(q_ws_lc), na=False)
            | token_contains
            | df["_nombre_lc"].str.contains(re.escape(q_name), na=False)
        )
        if by_barras:
            mask |= (
...
class SearchView(ttk.Frame):
    ...
    def __init__(self, master, go_home_cb, initial_db_path: Optional[Path]):
        super().__init__(master)
        self.master.title("Buscador de Códigos — MERCADO HOUSE (v90)")
    ...
    def on_search(self, force_general=False):
        ...
        if len(codes) == 1:
            ...
            mask   = _build_search_mask(df, q, q_ws, q_norm, exact, by_barras)
            out    = df[mask].copy()
            if out.empty:
        ...
        for i, code in enumerate(codes):
            ...
            mask = _build_search_mask(df, code, code_ws, code_norm, exact, by_barras)
            sub  = df[mask].copy()
            if sub.empty:
                ...
                sub["__input"] = code
                sub["__pos"]   = i
                sub["__rank"]  = range(len(sub))
                frames.append(sub)

        out = (
            pd.concat(frames, ignore_index=True)
              .sort_values(["__pos", "__rank"])
              .drop(columns=["__pos", "__rank"])
            if frames else
...
class TivendoWindow(ttk.Frame):
    def __init__(self, master, listado_path: Optional[Path] = None, go_home_cb=None):
        super().__init__(master)
        self.master.title("Tivendo - Cambios masivos de precios (v90)")
        self.pack(fill="both", expand=True)
        self.go_home_cb = go_home_cb
...
class TivendoIngresoMasivoArticulosWindow(ttk.Frame):
    def __init__(self, master, catalogo_path: Optional[Path] = None, go_home_cb=None):
        super().__init__(master)
        self.master.title("Tivendo - Ingreso Masivo de Artículos (v90)")
        self.pack(fill="both", expand=True)
        self.go_home_cb = go_home_cb
